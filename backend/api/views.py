...
class OAuthHandler:
    ...
    def oauth_login(self, request):
        ...
        logger.debug("OAuth login started.")
        logger.debug(f"Request session before clearing: {dict(request.session.items())}")
        ...
        try:
            ...
            logger.debug(f"Session after setting: {dict(request.session.items())}")
            ...
            logger.debug(f"Redirecting to OAuth URL: {oauth_url}")
...
class DexAnalysisResponse():
    ...
    def generate_most_suitable_claim_response(request):
        try:
            ...
            print("REQUEST IN generate_most_suitable_claim_response: ", request)
